...
(
    ...
    --The below is same as saying forall other models in the domain they are not EQ to 1, 2 or 3
    AND NOT 
...
AND ( 
    FORALL model2, color1, price1, type. ( 
        NOT Printer(model2, color1, type, price1) -->true for all printers in the world not in our DB
    OR (
        EXISTS model1, prodtype, color2, price2. 
            Product(maker, model1, prodtype) 
            AND 
            Printer(model1, color2, type, price2)  -->true så continue, 1 false så stop
        )
    )
)
